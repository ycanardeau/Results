...
public class FlatMapErrorTests
{
	...
	private static Result<int, int> Sync(string e) =>
		e == "retry" ? Result.Ok<int, int>(0) : Result.Error<int, int>(e.Length);

	private static async Task<Result<int, int>> AsyncFn(string e)
	{
		await Task.Yield();
		return Sync(e);
	}
	...
	[Fact]
	public void Sync_Error_can_bind_to_error() =>
		Assert.Equal(BoundToError, Result.Error<int, string>("x").FlatMapError(Sync));

	// Variant A: Task source, sync selector.
	[Fact]
	public async Task TaskSource_SyncSelector_Ok() =>
		Assert.Equal(PassedThrough, await Task.FromResult(Ok).FlatMapError(Sync));

	[Fact]
	public async Task TaskSource_SyncSelector_Error() =>
		Assert.Equal(Recovered, await Task.FromResult(Err).FlatMapError(Sync));
	...
	public async Task SyncSource_AsyncSelector_Ok_preserves_value() =>
		Assert.Equal(PassedThrough, await Ok.FlatMapError(AsyncFn));

	[Fact]
	public async Task SyncSource_AsyncSelector_Error() =>
		Assert.Equal(Recovered, await Err.FlatMapError(AsyncFn));

	// Variant C: Task source, async selector.
	[Fact]
	public async Task TaskSource_AsyncSelector_Ok() =>
		Assert.Equal(PassedThrough, await Task.FromResult(Ok).FlatMapError(AsyncFn));

	[Fact]
	public async Task TaskSource_AsyncSelector_Error() =>
		Assert.Equal(Recovered, await Task.FromResult(Err).FlatMapError(AsyncFn));
}
