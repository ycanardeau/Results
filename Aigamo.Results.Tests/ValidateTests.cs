...
public class ValidateTests
{
	...
	public async Task AsyncPredicate_Ok_true()
		=> Assert.True(await Ok.Validate(async x => { await Task.Yield(); return x % 2 == 0; }));

	[Fact]
	public async Task AsyncPredicate_Error_false()
		=> Assert.False(await Err.Validate(async _ => { await Task.Yield(); return true; }));
}
...
public class ValidateErrorTests
{
	...
	[Fact]
	public void Error_passing_predicate_is_true() => Assert.True(Err.ValidateError(e => e.StartsWith("bo")));

	[Fact]
	public void Error_failing_predicate_is_false() => Assert.False(Err.ValidateError(e => e.StartsWith("xx")));
	...
	public async Task AsyncPredicate_Error_true()
		=> Assert.True(await Err.ValidateError(async e => { await Task.Yield(); return e.Length == 4; }));
}
