...
public class GetOrTests
{
	...
	public async Task Async_Error_returns_fallback()
		=> Assert.Equal(4, await Err.GetOr(async e => { await Task.Yield(); return e.Length; }));

	[Fact]
	public async Task Async_Ok_returns_value()
		=> Assert.Equal(3, await Ok.GetOr(async _ => { await Task.Yield(); return -1; }));
}
...
public class GetErrorOrTests
{
	...
	public async Task Async_Ok_returns_fallback()
		=> Assert.Equal("v3", await Ok.GetErrorOr(async v => { await Task.Yield(); return "v" + v; }));
}
