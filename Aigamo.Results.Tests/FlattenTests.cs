...
public class FlattenTests
{
	...
	public void Outer_error_passes_through()
		=> Assert.Equal(Result.Error<int, string>("outer"), Result.Error<Result<int, string>, string>("outer").Flatten());

	[Fact]
	public async Task TaskSource_unwraps()
		=> Assert.Equal(Ok, await Task.FromResult(Result.Ok<Result<int, string>, string>(Ok)).Flatten());
}
