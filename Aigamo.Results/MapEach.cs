...
{
	public static Result<IEnumerable<U>, TError> MapEach<T, TError, U>(
		this Result<IEnumerable<T>, TError> source,
		Func<T, U> mapping
	) => source.Map(items => items.Select(mapping));

	// Task source, sync mapping.
	public static async Task<Result<IEnumerable<U>, TError>> MapEach<T, TError, U>(
		this Task<Result<IEnumerable<T>, TError>> source,
		Func<T, U> mapping
	...
	public static async Task<Result<IEnumerable<U>, TError>> MapEach<T, TError, U>(
		...
		await source
			.Fold(
				onOk: async items =>
					Result.Ok<IEnumerable<U>, TError>(
						await Task.WhenAll(items.Select(mapping)).ConfigureAwait(false)
					),
				onError: error => Task.FromResult(Result.Error<IEnumerable<U>, TError>(error))
			)
